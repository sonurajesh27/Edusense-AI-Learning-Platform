...
class GestureDataCollector {
  ...
  /**
   * Generate a unique ID with high precision timestamp + random string + counter
   */
  generateUniqueId() {
    // Use performance.now() for sub-millisecond precision + random string + incrementing counter
    const timestamp = performance.now().toString().replace('.', '');
    const random = Math.random().toString(36).substr(2, 9);
    const counter = (this._idCounter = (this._idCounter || 0) + 1);
    return `gesture_${timestamp}_${random}_${counter}`;
  }
